...
from langraph_day_1.tools.weather import get_weather, WeatherCode
...
async def event_astream():
    input_messages = {"messages": ["北京天气怎么样，北京的城市编码为:101010100"]}
    graph = create_react_agent(chat_model, tools)
    async for event in graph.astream_events(input=input_messages, version="v2"):
        kind = event["event"]
        print(f"{kind} : {event['name']}")

if __name__ == '__main__':
    asyncio.run(event_astream())